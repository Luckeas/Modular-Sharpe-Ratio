...
def generate_performance_charts(portfolio_series, trades, output_dir, date_range_str):
    ...
    # 4. Trade Duration Analysis - Bar Chart
    if trades:
        # Get bars held for all trades
        bars_held = [t['bars_held'] for t in trades]

        # Create histogram of bars held
        plt.figure(figsize=(12, 6))
        plt.hist(bars_held, bins=20, alpha=0.7, color='blue')
        plt.axvline(x=np.mean(bars_held), color='red', linestyle='--',
                    label=f'Mean: {np.mean(bars_held):.2f} bars')
        plt.title(f'Distribution of Trade Duration (5-minute bars) - {date_range_str}')
        plt.xlabel('Number of Bars Held')
        plt.ylabel('Frequency')
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.savefig(os.path.join(output_dir, f"trade_duration_distribution_{date_range_str}.png"))
        plt.close()

        # 5. Intraday vs. Overnight Trade Analysis
        # Categorize trades
        intraday_trades = [t for t in trades if t['entry_time'].date() == t['exit_time'].date()]
        overnight_trades = [t for t in trades if t['entry_time'].date() != t['exit_time'].date()]

        # Skip if one category has no trades
        if intraday_trades and overnight_trades:
            # Calculate metrics
            categories = ['Intraday', 'Overnight']
            win_rates = [
                sum(1 for t in intraday_trades if t['profit'] > 0) / len(intraday_trades) * 100,
                sum(1 for t in overnight_trades if t['profit'] > 0) / len(overnight_trades) * 100
            ]
            avg_profits = [
                sum(t['profit'] for t in intraday_trades) / len(intraday_trades),
                sum(t['profit'] for t in overnight_trades) / len(overnight_trades)
            ]
            counts = [len(intraday_trades), len(overnight_trades)]
            avg_bars = [
                np.mean([t['bars_held'] for t in intraday_trades]),
                np.mean([t['bars_held'] for t in overnight_trades])
            ]
            ...
            # Win rate subplot
            plt.subplot(2, 2, 1)
            plt.bar(categories, win_rates, color=['skyblue', 'navy'])
            ...
            # Profit subplot
            plt.subplot(2, 2, 2)
            plt.bar(categories, avg_profits, color=['skyblue', 'navy'])
            plt.title('Average Profit Comparison')
            plt.ylabel('Average Profit ($)')
            for i, v in enumerate(avg_profits):
                plt.text(i, v + (5 if v >= 0 else -5), f"${v:.2f}", ha='center')
            plt.grid(True, alpha=0.3)

            # Count subplot
            plt.subplot(2, 2, 3)
            plt.bar(categories, counts, color=['skyblue', 'navy'])
            ...
            # Bars held subplot
            plt.subplot(2, 2, 4)
            plt.bar(categories, avg_bars, color=['skyblue', 'navy'])
            plt.title('Average Bars Held')
            plt.ylabel('Bars')
            for i, v in enumerate(avg_bars):
                plt.text(i, v + 0.2, f"{v:.1f}", ha='center')
            plt.grid(True, alpha=0.3)

            plt.suptitle(f'Intraday vs. Overnight Trading Performance - {date_range_str}', fontsize=16)
            plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust for suptitle
            plt.savefig(os.path.join(output_dir, f"intraday_vs_overnight_{date_range_str}.png"), dpi=150)
            plt.close()

        # 6. Trade Profit vs Duration Scatter Plot
        plt.figure(figsize=(12, 6))
        # Separate intraday and overnight for different colors
        if intraday_trades:
            plt.scatter([t['bars_held'] for t in intraday_trades],
                        [t['profit'] for t in intraday_trades],
                        alpha=0.6, label='Intraday', color='blue')
        if overnight_trades:
            plt.scatter([t['bars_held'] for t in overnight_trades],
                        [t['profit'] for t in overnight_trades],
                        alpha=0.6, label='Overnight', color='red')

        plt.axhline(y=0, color='black', linestyle='-', alpha=0.3)
        plt.title(f'Trade Profit vs. Duration - {date_range_str}')
        plt.xlabel('Number of Bars Held')
        plt.ylabel('Profit/Loss ($)')
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.savefig(os.path.join(output_dir, f"profit_vs_duration_{date_range_str}.png"), dpi=150)
        plt.close()
